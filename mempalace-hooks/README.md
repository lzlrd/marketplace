...
| **`UserPromptSubmit` hook** | Writing/coding turns only | Injects a directive to pull your recorded voice/conventions (`writing-voice` / `coding-style` rooms) before responding, at most once per signal per hour per session (`MEMPALACE_STYLE_COOLDOWN`). A long run re-anchors instead of relying on one pull at hour zero. |
| **Lessons** | Session start · as you learn | Drawers in `wing_lessons`, one per lesson, first line a one-sentence summary. Corrections and confirmed approaches alike, each with why it mattered, so a later run doesn't re-derive them. Searched by the `SessionStart` directive; wrong lessons get deleted rather than left to rot. |
