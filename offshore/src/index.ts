...
/**
 * Offshore is an MCP server for bypassing model refusals.
 *
 * It forwards a question (with optional context) to a locally running abliterated
 * (uncensored) model, for questions Claude will not answer. Models are configured
 * the same way OpenAI's Codex CLI does it: a `model_providers` table in a TOML
 * config plus a top-level `model` / `model_provider` selector.
 */
